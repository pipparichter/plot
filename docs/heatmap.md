...
names must be valid, i.e. in the list `pop['filtered_genes']`. `None` by default.
* `diffexp_data` **dict** A structure containing information on differentially-expressed genes. This dictionary
must be generated by the `plot.get_diffexp_data()` function. See the **Unsupervised gene selection** section for more
information. 
...
HeatmapPlots can accept a curated list of genes, passed as the `genes` keyword argument. If no genes are specified, then
the `plot.get_diffexp_data()` is automatically called, with the filter relevant to the specific heatmap (e.g. `refpop=2` or
`celltype='T cell'`). More information on this function is in the [plot](./plot.md) 
section. The resulting list of highly up and down-regulated genes generated by this function is used in place of a curated
gene list.
